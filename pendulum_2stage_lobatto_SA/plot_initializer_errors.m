function [alpha beta] = plot_initializer_errors(H, errs, plot_errs_params)
% function [alpha beta] = plot_initializer_errors(H, errs, plot_errs_params)
...
% where x_i = log10(errs(i)) and y_i = log10(H(i)) and returns [alpha, beta],
...
%
% Inputs:
%	H: vector of stepsize values (just h, not rh)
...
x_reg = linspace(min(log10(H)),max(log10(H)));
y_reg = alpha + beta * x_reg;

%% print the line
fprintf('\nLeast squares line for ')
fprintf(print_str)
fprintf(' : log10(errs) = %0.2f + %0.2f * log10(H)\n', alpha, beta)
